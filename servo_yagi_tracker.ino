...
void handleManual() {
  ...
  // ОДНОВРЕМЕННО с активным фоновым BLE-сканом валит плату — конфликт LEDC
  // с тайминг-критичным NimBLE-стеком (см. подробный комментарий у
  // writePulseUs про то, почему это НЕ лечится критической секцией).
  // Пауза скана с подтверждением isScanning()==false снижает частоту сбоя
  // на порядок (с "падает через 1-2 резких джога" до "падает уже далеко не
  // на каждой пачке из ~10 подряд экстремальных прыжков позиции с интервалом
  // ~150 мс") — не идеально (это смягчение реальной race-condition в
  // связке SDK, а не полное её устранение), но кратно надёжнее, чем ничего.
  // Честно предупреди пользователя: очень резкое/частое дёрганье джог-
  // слайдера из края в край всё ещё теоретически может уронить плату; она
  // сама перезагрузится через Task Watchdog и восстановится за секунды.
